...
$(() => {
  ...
  // -- Carrier ranges costs --
  // Retrieve first the current selected zones
  const $selectedZones = $('#carrier_shipping_settings_zones');

  // On selected zones change, we emit an event to update the zones (with ranges data) for components that need it
  $selectedZones.on('change', () => {
    const zones = $selectedZones.val() || [];
    const ranges = JSON.parse(($('#' + ComponentsMap.carrierRanges.rangesSelectionDataId).val() || '{}').toString());
    eventEmitter.emit(CarrierFormEventMap.zonesUpdated, { zones, ranges });
  });
